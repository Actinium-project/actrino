...
type ChainSource interface {
	...
	GetBlock(chainhash.Hash, ...QueryOption) (*acmutil.Block, error)
...
func WatchAddrs(watchAddrs ...acmutil.Address) RescanOption {
	return func(ro *rescanOptions) {
...
func extractBlockMatches(chain ChainSource, ro *rescanOptions,
	curStamp *headerfs.BlockStamp) ([]*acmutil.Tx, error) {
	...
	blockDetails := acmjson.BlockDetails{
		Height: block.Height(),
		Hash:   block.Hash().String(),
		Time:   blockHeader.Timestamp.Unix(),
	}

	relevantTxs := make([]*acmutil.Tx, 0, len(block.Transactions()))
...
func (ro *rescanOptions) spendsWatchedInput(tx *acmutil.Tx) bool {
...
func (ro *rescanOptions) paysWatchedAddr(tx *acmutil.Tx) (bool, error) {
...
// replacement for the acmd rescan and notification functionality used in
...
func AddAddrs(addrs ...acmutil.Address) UpdateOption {
