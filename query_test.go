...
// a slice of them.
//
// NOTE: copied from Actinium-project/acmd/database/ffldb/interface_test.go
func loadBlocks(t *testing.T, dataFile string, network wire.BitcoinNet) (
	...
	blocks := make([]*acmutil.Block, 0, 256)
	genesis := acmutil.NewBlock(chaincfg.MainNetParams.GenesisBlock)
	...
	for height := 1; ; height++ {
		...
		block, err := acmutil.NewBlockFromBytes(blockBytes)
		if err != nil {
