...
import (
	...
	"github.com/Actinium-project/acmd/peer"
	...
	"github.com/Actinium-project/actrino/query"
)
...
var (
	...
	// DefaultFilterCacheSize is the size (in bytes) of filters actrino
	// will keep in memory if no size is specified in the actrino.Config.
	...
	// DefaultBlockCacheSize is the size (in bytes) of blocks actrino will
	// keep in memory if no size is specified in the actrino.Config.
...
func (sp *ServerPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > acmutil.MaxSatoshi {
		log.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, acmutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
type Config struct {
	// DataDir is the directory that actrino will store all header
	// information within.
	DataDir string
	...
	// compliant with the expected filter header state. If actrino starts
