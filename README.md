...
[![Build Status](https://travis-ci.org/Actinium-project/actrino.svg?branch=master)](https://travis-ci.org/Actinium-project/actrino)
[![Godoc](https://godoc.org/github.com/Actinium-project/actrino?status.svg)](https://godoc.org/github.com/Actinium-project/actrino)
[![Coverage Status](https://coveralls.io/repos/github/Actinium-project/actrino/badge.svg?branch=master)](https://coveralls.io/github/Actinium-project/actrino?branch=master)
...
#### Rescan
`Rescan` allows a wallet to scan a chain for specific TXIDs, outputs, and addresses. A start and end block may be specified along with other options. If no end block is specified, the rescan continues until stopped. If no start block is specified, the rescan begins with the latest known block. While a rescan runs, it notifies the client of each connected and disconnected block; the notifications follow the [acmjson](https://github.com/Actinium-project/acmd/blob/master/acmjson/chainsvrwsntfns.go) format with the option to use any of the relevant notifications. It's important to note that "recvtx" and "redeemingtx" notifications are only sent when a transaction is confirmed, not when it enters the mempool; the client does not currently support accepting 0-confirmation transactions.
