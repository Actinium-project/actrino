...
import (
	...
	"github.com/Actinium-project/acmwallet/wallet/txauthor"
...
var (
	...
	// messages.
	numQueryThreads = 20
	queryOptions    = []actrino.QueryOption{}
	...
	ourKnownTxsByBlock = make(map[chainhash.Hash][]*acmutil.Tx)

	// ourKnownTxsByFilteredBlock lets the rescan goroutine keep track of
	// transactions we're interested in that are in the blockchain we're
	// following as signalled by OnFilteredBlockConnected and
	// OnFilteredBlockDisconnected.
	ourKnownTxsByFilteredBlock = make(map[chainhash.Hash][]*acmutil.Tx)
)

// secSource is an implementation of acmwallet/txauthor/SecretsSource that
...
func (s *secSource) add(privKey *btcec.PrivateKey) (acmutil.Address, error) {
	pubKeyHash := acmutil.Hash160(privKey.PubKey().SerializeCompressed())
	addr, err := acmutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.params)
	if err != nil {
		return nil, err
	}
...
func (s *secSource) GetKey(addr acmutil.Address) (*btcec.PrivateKey, bool,
	error) {
...
func (s *secSource) GetScript(addr acmutil.Address) ([]byte, error) {
...
type actrinoHarness struct {
	h1, h2, h3 *rpctest.Harness
	svc        *actrino.ChainService
}

type syncTestCase struct {
	name string
	test func(harness *actrinoHarness, t *testing.T)
}
...
func testInitialSync(harness *actrinoHarness, t *testing.T) {
...
var (
	...
	errChan                   <-chan error
	rescan                    *actrino.Rescan
	startBlock                headerfs.BlockStamp
	secSrc                    *secSource
	addr1, addr2, addr3       acmutil.Address
...
// smaller tests.
func testRescan(harness *actrinoHarness, t *testing.T) {
	...
	spendReport, err := harness.svc.GetUtxo(
		actrino.WatchInputs(actrino.InputWithScript{
			PkScript: script1,
			OutPoint: ourOutPoint,
		}),
		actrino.StartBlock(&headerfs.BlockStamp{Height: 1101}),
	)
	if err != nil {
...
func testStartRescan(harness *actrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target acmutil.Amount) (
		total acmutil.Amount, inputs []*wire.TxIn,
		inputValues []acmutil.Amount, scripts [][]byte, err error) {
		ourIndex := 1 << 30 // Should work on 32-bit systems
		...
		return func(target acmutil.Amount) (total acmutil.Amount,
			inputs []*wire.TxIn, inputValues []acmutil.Amount,
			...
			inputValues = []acmutil.Amount{
				acmutil.Amount(tx.TxOut[ourIndex].Value)}
	...
	err = rescan.Update(actrino.AddAddrs(addr2), actrino.Rewind(1095))
	if err != nil {
	...
	_, err = harness.h1.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
		[]*acmutil.Tx{}, rpctest.BlockVersion, time.Time{},
		[]wire.TxOut{{
			Value:    0,
			PkScript: []byte{},
		}})
	...
	spendReport, err := harness.svc.GetUtxo(
		...
		actrino.StartBlock(&headerfs.BlockStamp{Height: 801}),
	)
	if err != nil {
		t.Fatalf("Couldn't get UTXO %s: %s", ourOutPoint, err)
	}
...
func testRescanResults(harness *actrinoHarness, t *testing.T) {
...
// TODO: Make this a benchmark instead.
func testRandomBlocks(harness *actrinoHarness, t *testing.T) {
...
func TestNeutrinoSync(t *testing.T) {
	...
	// Create a second acmd SimNet node
	h2, err := rpctest.New(
		&chaincfg.SimNetParams, nil, []string{"--txindex"},
	)
	if err != nil {
	...
	// Create a third acmd SimNet node and generate 1200 blocks
	h3, err := rpctest.New(
		&chaincfg.SimNetParams, nil, []string{"--txindex"},
	)
	if err != nil {
	...
	actrino.MaxPeers = 3
	actrino.BanDuration = 5 * time.Second
	actrino.QueryPeerConnectTimeout = 10 * time.Second
	svc, err := actrino.NewChainService(config)
	if err != nil {
	...
	// Create a test harness with the three nodes and the actrino instance.
	testHarness := &actrinoHarness{h1, h2, h3, svc}
...
func waitForSync(t *testing.T, svc *actrino.ChainService,
...
func startRescan(t *testing.T, svc *neutrino.ChainService, addr acmutil.Address,
	...
	rescan := neutrino.NewRescan(
		...
		neutrino.NotificationHandlers(
			rpcclient.NotificationHandlers{
				...
				OnRedeemingTx: func(tx *acmutil.Tx,
					details *acmjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
...
func banPeer(t *testing.T, svc *actrino.ChainService, harness *rpctest.Harness) {
	t.Helper()
